...
namespace EnhancedTriggerbox
{
    ...
    public class EnhancedTriggerBox : MonoBehaviour
    {
        ...
        /// </summary>
        [SerializeField] private List<EnhancedTriggerBoxComponent> conditions = new List<EnhancedTriggerBoxComponent>();

        /// <summary>
        /// Holds all the responses the user has added to the trigger box
        /// </summary>
        [SerializeField] private List<EnhancedTriggerBoxComponent> responses = new List<EnhancedTriggerBoxComponent>();
        ...
        /// </summary>
        [Tooltip(
            "If true, the script will write to the console when certain events happen such as when the trigger box is triggered.")]
        ...
        /// </summary>
        [Tooltip(
            "If this is true, you won't see any warnings in the editor when you're missing references or if there's something which could cause an error.")]
        ...
        /// </summary>
        [Tooltip(
            "This is the total time that the conditions must be met for in seconds before the responses get executed.")]
        ...
        /// </summary>
        [Tooltip(
            "This is used when Trigger Follow is set to Follow Transform. The trigger box will stay positioned on wherever this transform is currently positioned.")]
        ...
        /// </summary>
        [Tooltip(
            "This is can be used if you cannot get a reference for the above Follow Transform transform. GameObject.Find() will be used to find the gameobject and transform with the name you enter.")]
        ...
        private void DisplayComponents(List<EnhancedTriggerBoxComponent> components)
        {
            ...
            for (int i = components.Count - 1; i >= 0; i--)
            {
                if (components[i])
                {
                    if (components[i].deleted
                    ) // If the X button has been pressed in the top right corner of a component
                    {
        ...
        private void DisplayAddComponent(string componentType, string[] componentNames,
            List<EnhancedTriggerBoxComponent> componentList)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (conditionIndex != 0)
                {
                    ...
                    if (conType == null)
                    {
                        Debug.Log("Unable to find the " + componentType + " " +
                                  componentNames[conditionIndex].Replace(" ", "").ToString() +
                                  ". Make sure it has the EnhancedTriggerbox.Component namespace.");
                    }
                    else
                    {
        ...
        void Start()
        {
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(followTransformName))
                {
                    ...
                    {
                        Debug.Log("Unable to find game object" + followTransformName +
                                  " for Trigger Follow. Reverting to static.");
        ...
        private IEnumerator ConditionsMet()
        {
            ...
            switch (afterTrigger)
            {
                ...
                case AfterTriggerOptions.DoNothing
                    : // If do nothing is set, simply reset the variables and all the components
                    conditionTimer = 0f;
        ...
        private void OnTriggerExit(Collider other)
        {
            ...
            {
                if (afterTrigger == AfterTriggerOptions.ExecuteExitResponses
                ) // If we have been told to do something on exit
                {
        ...
        private void OnDrawGizmos()
        {
            ...
            if (!disableEntryCheck)
            {
                ...
                Gizmos.DrawCube(
                    new Vector3(etbCollider.bounds.center.x, etbCollider.bounds.center.y, etbCollider.bounds.center.z),
